...
| 6 | CI 构建效率与资源预算 | 分阶段耗时、资源预算检查设计 | 两轮真实 CI 已核验；warn/strict 预算已通过本地验证，待远程失败/恢复验收，见[记录](../performance/phase-6-ci-budget.md) |
...
本轮进度：用户手动完成真实 PR 的两轮运行与下载，记录均已核验。npm 缓存未命中/命中时，安装分别 20 s/8 s，构建均 9 s；全量 JS 250399 B、CSS 22965 B，逐文件记录一致。不宣称稳定提速。JS/CSS 预算、warn/strict 脚本与工作流接入已通过本地验证；日常超线提醒，严格模式失败，待用户手动验证远程失败与恢复。见[阶段 6 记录](../performance/phase-6-ci-budget.md)。后端接口精简不在本轮范围。
