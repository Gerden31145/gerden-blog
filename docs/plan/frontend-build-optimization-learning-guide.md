...
| 3 | Tailwind 与页面 CSS 优化 | CSS 产物对比、视觉回归结果 | 已验收并保留；阶段 7 用户最终人工检查补齐页面 style 验收 |
...
| 6 | CI 构建效率与资源预算 | 分阶段耗时、资源预算检查设计 | 已验收；用户完成临时 PR 与记录收尾并反馈检查通过，见[记录](../performance/phase-6-ci-budget.md) |
| 7 | 复现、复盘与面试表达 | 最终报告、证据索引、可核验简历表述 | A/B 与人工回归完成，已有简历表述；待报告提交及原 PR 发布收尾，见[成果记录](../performance/phase-7-results.md) |
...
本轮进度：用户手动完成真实 PR 的两轮基线运行与下载，记录均已核验。npm 缓存未命中/命中时，安装分别 20 s/8 s，构建均 9 s，不宣称稳定提速。全量 JS 250399 B、CSS 22965 B；日常 warn 通过、临时 strict 超线失败且保留附件、恢复阈值后 strict 通过的报告均已核验，资源逐文件一致。待用户确认恢复运行整体状态，关闭临时验证 PR 并回到学习分支提交记录。见[阶段 6 记录](../performance/phase-6-ci-budget.md)。后端接口精简不在本轮范围。
...
6. 预算初期先报告。本项目经讨论采用日常 warn，初始 5% 仅作复查线，strict 用于受控失败验证；新功能成本应评审，阈值不自动增长。使用业务角色或 manifest 解析定位资源，避免绑定每次变化的哈希文件名。
...
**验收：** 用临时验证分支的 strict 模式和一次受控超预算改动确认检查会失败且保留报告，撤回该验证改动后严格检查通过；日常保留 warn。报告能够关联 commit/release，原发布健康检查仍有效。

**追问：** 为什么缓存命中但构建没有明显变快？预算如何避免误报？为什么不能根据文件名中的 hash 追踪逻辑模块？

## 阶段 7：整理能复现的成果与面试答案

当前从已提交学习版本 2e2c906 开始整理；阶段 6 已收尾。先完成成果表与口径解释，再确定整体 A/B 复测条件，不把历史单项实验直接包装成最终版本的整体收益。
