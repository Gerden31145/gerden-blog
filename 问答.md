...
**要点**:
1. **组件结构**: `AdminPostModal.vue`，props 包含 `visible`（是否显示）和 `post`（编辑时传入已有数据），emit 包含 `close` 和 `success`
2. **表单字段**: 标题、摘要、slug、标签（逗号分隔）、状态（draft/published/hidden）、Markdown 文件上传，与 `createPost` 的 Zod schema 对应
3. **提交方式**: 使用 `FormData`，`file` 字段放 md 文件，`meta` 字段放 JSON 字符串（含 title/summary/slug/post_status/tags）
4. **父组件集成**: `admin/index.vue` 中维护 `modalVisible` 和 `editingPost` 状态，"+"按钮打开新增弹窗，UPDATE 按钮打开编辑弹窗，提交成功后重新拉取列表
...
2. **标准写法**: `onMounted` 里 `setInterval`，`onUnmounted` 里 `clearInterval`，两边都只在客户端执行，SSR 完全不涉及
3. **composable 场景**: 推荐用 `onScopeDispose` 代替 `onUnmounted`，它绑定的是 Vue 的 effect scope 而非组件实例，更适合 composable 夽数
4. **核心原则**: 所有浏览器 API（`setInterval`、`setTimeout`、`document` 等）都放在 `onMounted` 里启动，`onUnmounted` / `onScopeDispose` 里清理
...
**问题**: 对 `worker/src/types/app.ts` 中的 `AppEnv` 类型有疑惑：它是不是 Hono 的请求上下文？`AuthUser` 是否用于判断用户是否登录？用户状态是如何维护的，难道每次请求都先经过 `/api/me` 判断登录状态并写入 `AuthUser` 吗？进一步理解是否可以概括为：中间件解析 cookie 并将状态写入请求上下文，后续业务直接调用本次请求上下文就可以知道用户是否登录？

**回答摘要**: `AppEnv` 不是请求上下文本身，而是传给 Hono 的类型参数，用来描述 `c.env` 和 `c.set()` / `c.get()` 中可用变量的类型。`AuthUser` 表示本次请求中已认证用户的最小身份信息。用户登录状态不是由 `/api/me` 写入后端上下文长期维护的，而是每个需要认证的请求都由 auth middleware 从 HttpOnly cookie 中读取并验证 JWT / session，再把当前用户写入本次请求上下文。`/api/me` 只是给前端恢复或确认登录状态用的普通接口。

**要点**:
1. **`AppEnv` 的作用**: `AppEnv` 是 `new Hono<AppEnv>()` 的类型声明，用来告诉 TypeScript 当前 Hono 应用有哪些 `Bindings` 和 `Variables`
2. **`Bindings` 是什么**: `Bindings` 描述 `c.env`，例如 D1 绑定、`JWT_SECRET` 等 Worker 环境变量
3. **`Variables.user` 是什么**: `Variables.user` 描述 `c.set('user', authUser)`、`c.get('user')`、`c.var.user` 中 `user` 的类型
4. **`AuthUser` 是什么**: `AuthUser` 是本次请求里的认证用户快照，通常包含 `id`、`sessionId`、`role`、`tokenVersion`
5. **请求上下文不是持久状态**: Hono 的 `Variables` 只存在于单次请求生命周期，请求结束后就消失；下一次请求会重新解析 cookie 并重新写入
6. **`/api/me` 的作用**: `/api/me` 用于前端刷新页面后向后端确认当前 cookie 是否仍然有效，并把返回的用户信息写回 Pinia；它不是后端登录状态的来源
7. **后端登录态来源**: 后端真正判断登录状态依赖浏览器自动携带的 HttpOnly cookie、JWT 校验、D1 session 校验、用户状态校验和 `token_version` 校验
8. **当前项目状态**: 目前 `worker/src` 中还没有实际实现 auth routes 和 auth middleware，`AuthUser` 现在是后续迁移认证模块时要使用的类型约定
...
用户登录
  ...
  -> 签发 access JWT
  -> 写入 HttpOnly cookie
  -> 前端 Pinia 保存展示用 user 信息

后续请求
  -> 浏览器自动携带 cookie
  -> auth middleware 读取 cookie 中的 JWT
  -> 校验 JWT / session / token_version / 用户状态
  -> c.set('user', AuthUser)
  -> 后续 route handler 用 c.get('user') 获取当前用户
```
...
app.post('/api/posts/:id/comments', requireAuth, async (c) => {
  ...
  // 这里一定有 user，可以用 user.id 创建评论
})
```

**可选登录态场景**:

有些接口可以允许未登录访问，但如果登录了就读取用户信息。这类场景可以做 `optionalAuth`：

```ts
app.get('/api/posts/:id/comments', optionalAuth, async (c) => {
  const user = c.get('user')
...
如果项目需要支持 `optionalAuth`，`AppEnv` 中的 `user` 类型更稳妥地写成可选：
...
如果只在 `requireAuth` 之后的路由中读取 `user`，也可以保持：
...
**结论**: 是的，正确理解就是：中间件在每次请求中解析 cookie 并验证登录态，然后把当前用户写入 Hono 的本次请求上下文；后续业务代码直接从这个请求上下文读取用户信息即可判断用户是否登录和是否有权限。
